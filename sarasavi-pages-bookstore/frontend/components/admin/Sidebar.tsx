...
export default function Sidebar({ isMobileOpen = false, onClose }: SidebarProps = {}) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-72 bg-[#FFFFFF] border-r border-[#E2E7D8] flex flex-col justify-between h-full shadow-2xl transition-transform duration-300 ease-in-out font-sans
        lg:static lg:w-64 lg:h-screen lg:sticky lg:top-0 lg:shadow-xs lg:translate-x-0
        ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
      `}>
        {/* Brand Header */}
        <div>
          <div className="p-4 sm:p-5 border-b border-[#E2E7D8] flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-2xl bg-[#34451D] flex items-center justify-center text-white flex-shrink-0 shadow-xs">
              ...
              <div>
                ...
                <span className="text-[11px] text-[#707365] font-medium mt-1 block">Admin Control Hub</span>
              </div>
            </div>
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded-xl text-[#707365] hover:text-[#20231B] hover:bg-[#F0F4E8] lg:hidden"
        ...
        {/* User Role Banner */}
        <div className="px-4 py-3 mx-3 my-3 rounded-2xl bg-[#F8F9F5] border border-[#E2E7D8] shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-[10px] uppercase font-semibold tracking-wider text-[#707365] font-mono">
              Current Access
            </span>
            <span className={`text-[10px] font-sans px-2 py-0.5 rounded-full font-bold ${
              user.role === 'SUPER_ADMIN' 
                ? 'bg-[#EBF0E4] text-[#34451D] border border-[#DCE3D2]' 
                : 'bg-[#EBF0E4] text-[#20231B]'
            }`}>
        ...
        <nav className="px-3 space-y-1 mt-2">
          <div className="px-3 py-1.5 text-[11px] uppercase font-semibold tracking-wider text-[#707365] font-mono">
            ...
            return (
              <Link
                ...
                className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-medium transition-all group ${
                  isActive
                    ? 'bg-[#34451D] text-white shadow-sm font-semibold'
                    : 'text-[#20231B] hover:text-[#34451D] hover:bg-[#F0F4E8] font-medium'
                }`}
              >
                <div className="flex items-center gap-3">
                  <Icon className={`w-4 h-4 transition-colors ${
                    isActive ? 'text-[#B7D85A]' : 'text-[#707365] group-hover:text-[#596B32]'
                  }`} />
                ...
                {item.badge && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono font-semibold ${
                    isActive
                      ...
                      : 'bg-[#F0F4E8] text-[#34451D] border border-[#DCE3D2]'
                  }`}>
                    {item.badge}
                  </span>
                )}
      ...
      <div className="p-3 border-t border-[#E2E7D8] space-y-1">
        <Link
          href="/"
          onClick={handleLinkClick}
          className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-medium text-[#20231B] hover:text-[#34451D] hover:bg-[#F0F4E8] transition-all"
        >
          <div className="flex items-center gap-2.5">
            <Globe className="w-4 h-4 text-[#596B32]" />
            <span>Customer Storefront</span>
          </div>
          <ChevronRight className="w-3.5 h-3.5 text-[#707365]" />
        </Link>
