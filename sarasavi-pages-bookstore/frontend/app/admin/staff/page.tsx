...
const ROLES: { value: StaffRole; label: string; desc: string; color: string }[] = [
  { 
    ...
    color: 'bg-[#E4E7D2] text-[#34451D] border-[#7F9148] font-semibold'
  },
  { 
    value: 'PAYMENT_ADMIN', 
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-display font-light text-[#20231B] flex items-center gap-2.5">
            <Users className="w-6 h-6 text-[#596B32]" />
        ...
        <div
          className={`p-3.5 rounded-2xl flex items-center justify-between text-xs border font-medium ${
            feedback.type === 'success'
              ? 'bg-[#E4E7D2] border-[#7F9148] text-[#34451D]'
              : 'bg-rose-50 border-rose-300 text-rose-900'
          }`}
        >
          <div className="flex items-center gap-2">
            {feedback.type === 'success' ? <CheckCircle className="w-4 h-4 shrink-0 text-[#596B32]" /> : <AlertCircle className="w-4 h-4 shrink-0 text-rose-600" />}
            <span>{feedback.message}</span>
          </div>
          <button onClick={() => setFeedback(null)} className="hover:opacity-80 text-[#85887A]">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
                ...
                {filteredStaff.map((staff) => {
                  ...
                  return (
                    <tr key={staff.id} className="hover:bg-[#efead5]/50 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-[#85887A] text-xs font-normal">{staff.email}</td>
                      ...
                      <td className="py-3.5 px-4">
                        {staff.active ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-rose-100 text-rose-950 border border-rose-300 text-[11px] font-semibold font-sans">
                            <XCircle className="w-3.5 h-3.5 text-rose-700" /> Deactivated
                          </span>
                        )}
                      </td>

                      <td className="py-3.5 px-4 text-[#85887A] text-xs font-medium">
          ...
          <div className="bg-[#efead5] border border-[#CDD3B5] rounded-3xl p-6 max-w-lg w-full shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 text-[#20231B]">
            <div className="flex items-center justify-between pb-4 border-b border-[#CDD3B5]/60">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-sm font-semibold text-[#20231B]">Create Administrator Account</h2>
                  <p className="text-[11px] text-[#85887A] font-medium">Module 1 (Gunathilaka) - RBAC Provisioning</p>
                </div>
              </div>
              <button 
            ...
            <div className="mt-4 p-3.5 rounded-2xl bg-[#E4E7D2] border border-[#CDD3B5] space-y-1.5 text-[11px] text-[#34451D]">
              ...
              <ul className="space-y-1 text-[#34451D]/90 pl-5 list-disc">
                <li><strong className="text-[#20231B]">Username Rule:</strong> 3-30 characters, letters/numbers/underscores only.</li>
            ...
            <form onSubmit={handleCreateStaff} className="mt-4 space-y-3.5 text-xs">
              <div>
                <label className="block text-[#20231B] font-semibold mb-1">Full Legal Name *</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-[#20231B] font-semibold mb-1">Corporate Email Address *</label>
                <input
                  type="email"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="text-[#20231B] font-semibold">Username *</label>
                    <button
                      type="button"
                      onClick={handleAutoGenerateUsername}
                      className="text-[10px] text-[#596B32] hover:text-[#34451D] font-semibold hover:underline flex items-center gap-0.5"
                    >
                      <Sparkles className="w-2.5 h-2.5 text-[#596B32]" /> Auto-fill
                    </button>
                  </div>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[#20231B] font-semibold mb-1">Password (min 6 chars) *</label>
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#85887A] hover:text-[#20231B]"
                    >
              ...
              <div>
                <label className="block text-[#20231B] font-semibold mb-1">Role / Module Authority *</label>
                <select
                  value={addForm.role}
          ...
          <div className="bg-[#efead5] border border-[#CDD3B5] rounded-3xl p-6 max-w-md w-full shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 text-[#20231B]">
            <div className="flex items-center justify-between pb-4 border-b border-[#CDD3B5]/60">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-sm font-semibold text-[#20231B]">Edit Admin Account</h2>
                  <p className="text-[11px] text-[#596B32] font-semibold font-sans">@{selectedStaff.username}</p>
                </div>
              </div>
              <button 
            ...
            <form onSubmit={handleUpdateStaff} className="mt-4 space-y-3.5 text-xs">
              <div>
                <label className="block text-[#20231B] font-semibold mb-1">Full Name</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[#20231B] font-semibold mb-1">Username</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-[#20231B] font-semibold mb-1">Email</label>
                  <input
                    type="email"
                    required
              ...
              <div>
                <label className="block text-[#20231B] font-semibold mb-1">Role / Authority Level</label>
                <select
                  value={editForm.role}
              ...
              <div>
                <label className="block text-[#20231B] font-semibold mb-1">
                  Change Password <span className="text-[#85887A] font-normal">(Leave blank to keep existing)</span>
                </label>
                <div className="relative">
                  <input
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-[#efead5] border border-rose-200 rounded-3xl p-6 max-w-md w-full shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 text-[#20231B]">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-sm font-semibold text-[#20231B]">Permanently Delete Account?</h3>
                <p className="text-[11px] text-[#85887A] font-medium font-sans">ID #{selectedStaff.id}: @{selectedStaff.username}</p>
              </div>
            </div>

            <p className="text-xs text-[#85887A] mb-4 leading-relaxed font-normal">
              Are you sure you want to delete <strong className="text-[#20231B] font-semibold">{selectedStaff.fullName}</strong> (@{selectedStaff.username})? 
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-full bg-rose-600 hover:bg-rose-700 text-white font-semibold text-xs shadow-xs transition-colors disabled:opacity-50"
              >
